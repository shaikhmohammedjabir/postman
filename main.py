...
from tkinter import Tk, Label, Button, Entry, OptionMenu, Checkbutton, StringVar, BooleanVar, Text, W, DISABLED, NORMAL,END
...
class PostMan(Tk):
    def __init__(self):
        ...
        self.__packAndBind(Entry(self,name=URL_ENTRY),row=0,column=1,ipadx=200,ipady=8,pady=10);
        self.__packAndBindWithEventListener(event=Button(self, text=EXECUTE,name=BUTTON,bg=GREEN),function=self.__callAPI,row=0, column=2);

        #FOR TOKEN INPUT FIELD ENABLE OR DISABLE
        self.__packAndBind(Label(self,text=TOKEN),row=1,column=0);
        self.__packAndBindWithEventListener(event=Checkbutton(self,variable=self.token_radio_value,onvalue=DISABLED,offvalue=NORMAL),function=self.__radioONOFF,row=1,column=1,sticky=W);
        self.__packAndBind(Entry(self,name=TOKEN_ENTRY,state=DISABLED),row=2, column=1,ipady=8,ipadx=200,pady=10,sticky=W,columnspan=3);
        self.__packAndBind(Text(self,name=RESPONSE_TEXT),row=3,column=1);
